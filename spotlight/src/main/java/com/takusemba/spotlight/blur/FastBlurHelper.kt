 ...
 */
// all credits to
// https://github.com/tvbarthel/BlurDialogFragment/blob/master/lib/src/main/java/fr/tvbarthel/lib/blurdialogfragment/FastBlurHelper.java
internal object FastBlurHelper {
  /**
   * blur a given bitmap
   *
   ...
   */
  @SuppressLint("NewApi") fun doBlur(
      sentBitmap: Bitmap, radius: Int, canReuseInBitmap: Boolean
  ): Bitmap? {
    if (radius < 1) {
      return null
    }
    val bitmap: Bitmap
    bitmap = if (canReuseInBitmap || sentBitmap.config == Bitmap.Config.RGB_565) {
